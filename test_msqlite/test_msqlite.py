...
def test_msqlite_single_thread():
    ...
    with MSQLite(db_path, table_name, schema) as db:
        ...
        assert db.max_execution_time is not None and db.max_execution_time < 1.0  # 0.016998291015625 has been observed
...
def _get_mp_db_path():
    return Path(get_temp_dir(), "test_msqlite_multi_process.sqlite")


def _write_db(value: int):
    schema = {"value": int, "timestamp": float}
    with MSQLite(_get_mp_db_path(), table_name, schema) as db:
        ...
        if (retry_count := db.retry_count) > 0:
            print(f"{value=}:{retry_count=},{db.max_execution_time=}", flush=True)
        retry_count = db.retry_count
    return retry_count


def test_msqlite_multi_process():
    _get_mp_db_path().unlink(missing_ok=True)
